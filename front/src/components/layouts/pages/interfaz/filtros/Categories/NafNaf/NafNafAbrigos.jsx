...
function NafNafAbrigos() {
  const [allProducts, setAllProducts] = useState([]);
  const [total, setTotal] = useState(0);
  const [countProducts, setCountProducts] = useState(0);
  const [NafNafAbrigosProducts] = useState([]);
      ...
      <main id='Secciones' style={{ backgroundImage: "linear-gradient(0deg, #b8b8b8 0%, #182848 50%)"}}>
        ...
        <section className="Ropas">
          <FiltroDeRopas
            ...
            countProducts={countProducts}
            setCountProducts={setCountProducts}
            data={NafNafAbrigosProducts} // Pasa los productos de Nafnaf como prop
          /> 
        </section>
      </main>
      <Foother />
